...
class SkillsTest extends TestCase
{
    ...
    private const string SLUG_PATTERN = '#^(site|seo|user|config)-[a-z-]+$#';
    ...
    // The file a class name stands for, src/ being flat enough for the basename to be unique - a relative namespace ("Service\ConfigService") is resolved as written, a bare name is looked up
    ...
    public function testEveryClassMemberQuotedExists(): void
    {
        foreach ($this->loadSkills() as $directory => $content) {
            foreach ($this->backtickedTokens($content, '#^(?!c975L\\\\|App\\\\)[A-Z][A-Za-z]*(\\\\[A-Z][A-Za-z]*)*(::[A-Za-z_]+(\(\))?)?$#') as $token) {
                ...
                // Only what this bundle could hold: a namespaced name is checked when its first segment is a real folder of src/ ("Service\ConfigService"), a bare one when src/ holds a file of that name at all - Symfony's own "Assert\Email" or "FormBuilder::add()" are neither, and are no business of this repository
