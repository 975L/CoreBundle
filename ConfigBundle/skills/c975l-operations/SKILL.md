...
php bin/console c975l:skills:install         # links every bundle's agent skills into .claude/skills/
```
...
**Under `test` the limiter is prepended with `policy: no_limit`**: a functional suite drives its
whole run from one address and meets the sixtieth request where a visitor never would, so a site's
own tests would answer 429 to each other. The listener stays wired and still consumes — only the
refusal disappears, never the path a request runs through.
...
- **Do not run EasyAdmin's `easyadmin:ai:install` beside `c975l:skills:install`** — it declines to
  overwrite the link this one made, so run either, not both.
