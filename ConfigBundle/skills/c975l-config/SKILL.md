...
- **`feature: true`** — for the key switching a feature on (an API key, an OAuth client id): while
  empty, the entry is listed in the dashboard's "Not used on this site yet" panel. Never on a bool
  turned off on purpose, nor on an entry already carrying a `severity`, which is an alert already.
