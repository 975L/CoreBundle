...
`startStimulusApp()` statically imports every `enabled`+`eager` controller listed there, so leaving it on has two costs. On the **front-end**, your `app.js` pulls `chart.js` (~66 KiB transferred) onto every public page, where no chart is ever rendered. On the **`/management` dashboard**, the admin entries (see `DashboardController::configureAssets()`) share one Stimulus application, which registers the chart controller once from `controllers.json` and once explicitly — the barrel's own `chartjs:pre-connect` listener destroys a live `Chart` before a second one is built on the same `<canvas>`, which Chart.js otherwise rejects with *"Canvas is already in use"*.
...
Standing in the back office at all is `BackOfficeAccessVoter::ACCESS` (`C975L_ACCESS_BACK_OFFICE`), the floor the dashboard, the "What's new" page and the guided-project panel are gated by — every other screen states its own bar on top, and every block of the dashboard filters itself by role (menus, links, alerts, shortcuts, guided projects), so what a user gets is the part of the back office that is theirs rather than a 403 on the way in. It grants on any of `site-role-contributor`, `site-role-editor`, `site-role-admin` or `ROLE_SUPER_ADMIN`, **held outright**: no `role_hierarchy` is shipped (same reason as `UserManagementVoter` above), so a plain `denyAccessUnlessGranted($configService->get('site-role-editor'))` would lock out an account holding only the admin role. Use the attribute — not one of the config values — wherever a screen is open to the whole back office, EasyAdmin's `setPermission()` included, which goes through `isGranted()` just the same.

`site-role-contributor` (`ROLE_CONTRIBUTOR` by default, offered by `user-roles-available` out of the box) opens that floor and nothing else: each screen decides whether it lets a contributor in.

**Seeing the back office as a lower role.** The user menu carries a "View as" section — one entry per level below the account's own (admin, editor, contributor, member) — added by `DashboardController::configureUserMenu()`. `RolePreview` keeps the chosen level in the session only, the token keeping its real roles, and `RolePreviewRoleVoter` hands the previewed role to every `isGranted()`, `#[IsGranted]`, EasyAdmin permission and `access_control` rule. `RolePreviewRoleVoterPass` decorates whichever of Symfony's two role voters the security configuration kept, so a site declaring a `role_hierarchy` still compiles and still gets it applied. `<twig:c975LConfig:Security:RolePreviewBanner />` shows the level and the way back — UiBundle's layout renders it, and the back office appends it to every page. Previewing as a member leaves the back office, which is why `RolePreviewController`'s two routes (`config_role_preview_start`, `config_role_preview_stop`) live outside `/management`.
...
`Service\SiteLocales` is the one place anything asks what a site offers — `all()` and `isMultilingual()`. It always holds the default locale whether or not the list names it, and drops any code the Intl catalogue doesn't know, a typo otherwise taking down every back-office page through EasyAdmin's `Locale::new()`. `Listener\LocaleListener` (priority 20) then sets each request's language from the `_locale` query parameter, then the session, then what the browser asks for — a route carrying its own `_locale` attribute winning over all three. A `?_locale=xx` sent to such a route moves the visitor to that same route in the language asked (`/fr/preview/abc` → `/en/preview/abc`, a route Symfony declares per language included), keeping the choice in session; a route whose `_locale` is only a default is left where it is. `Service\LocalizedUrlGenerator::sameRouteIn()` builds that url, and `screenLanguages()` offers it in each language for a route with no localised twin.
...
**`warning`:** optional — says the current state deserves an admin's attention, and **it is the flag that paints the tile**: a warning tile wears `shortcut-tile-warning` (Bootstrap's subtle warning tokens, so it follows the admin theme), which is how an admin reads what the site currently has in a state worth noticing — maintenance on, registration closed, a bundle's test mode — without going through every label. Leave it out and `ShortcutBuilder` fills it from `active`, which is what a tile whose "on" is the state to signal wants: maintenance on is worth a colour, maintenance off is not. Set it yourself when the state to signal is the other one — `ConfigShortcutProvider`'s registration tile passes `'warning' => false` while registration is open, an open registration being what a site normally does, and `true` on a form that is closed **or was never seeded**, which otherwise looks exactly like a site that works.
...
**Films:** each project carries a `film` url, `EcosystemUrls::TUTORIAL_FILM` (`https://bundles.975l.com/tutoriels/film`) followed by its slug, shown as "Watch the film" beside it in the dashboard list and beside Donovan's citation of it. The address answers every slug, one never filmed landing on the films' index, so the link is written without knowing which films exist. A site publishing films of its own (SiteBundle's tutorials) implements `TutorialFilmUrlProviderInterface`: `getFilmUrl($slug)` returns its film's url, or null to leave that project on the ecosystem's. The dashboard's header links to the films' index, `EcosystemUrls::TUTORIALS`, beside the block showcase.
...
**It opens on what is left to act upon**, not on everything recorded: the status filter starts on *À traiter* — the warning and error rows nobody has declared dealt with — and a muted line under the table counts what that leaves out. A site whose checks pass records hundreds of *OK* rows, and a table listing them all beside the one thing to fix is a table nobody reads. Every row is still one click away, through the *Tous les statuts* option.

The `skipped` status reads **Non vérifiable**, not "not tested": it covers a page the check never reached *and* a target that is up and simply turns automated probes down — most stores answer `403` to a `HEAD` carrying no browser behind it. Neither is something an editor can fix, which is why those rows are left out of the default view along with the conforming ones, and why they are neutral rather than red.

**Marking a row as fixed**: the ✓ button on a warning or error row stamps its `acknowledgedAt` (`POST /management/health-check/{id}/acknowledge`, `site-role-admin` + CSRF token, answered as JSON and applied to the row in place). It leaves the default view and stops counting towards the dashboard alert immediately — what an admin who has just fixed something needs, a full run costing minutes and hitting every external platform again. Clicking it a second time takes the acknowledgement back.

> The acknowledgement is borne by **the row**, never by the (url, kind) pair. Rows are only ever appended, so the next run records a fresh unacknowledged one: a problem that was not actually fixed comes straight back on its own, and nothing can be buried for good. Acknowledged rows stay in the table under *Tous les statuts* and keep their place in the CSV export, which gains an `acknowledgedAt` column rather than dropping them — the audit trail must not lose what an admin decided.
...
**A failed mirror is carried onto the backup row** by the next `c975l:config:backup`, as a warning and a line of its report. The mirror runs on its own night and used to record its failures where nothing read them: the archives push refreshes the timestamp the offsite status is computed from every six hours, so a mirror red since January sat under a row saying "offsite ok". Only the mirror is read back this way — the archives push is that command's own and is reported the moment it fails. What the row carries is the first 200 characters of rclone's message, folded onto one line; the whole of it is in `var/backup/.offsite.json`.
...
`c975l:config:users-cleanup` warns then anonymizes the accounts left unused, the GDPR asking not to keep personal data without a limit. An account nobody logged in to for `user-inactivity-days` minus `user-inactivity-notice-days` (1095 and 30 by default, both `restricted`) is emailed the `account_inactivity_notice` template, editable from the back-office; one still unused once the notice is over is anonymized. A disabled account (banned, or never confirmed) cannot log in, so it is anonymized without being warned. `user-inactivity-days` at `0` keeps every account, and a notice not shorter than the inactivity stops the run with an error.
...
```json
{
    ...
    "extra": {
        ...
        "messenger": {"transports": {"async": 0, "failed": 28}, "failureTransport": "failed"}
    }
}
```
...
**The three sections this bundle contributes to `extra`** answer the questions a console holding a dozen sites can't get from a version number. `capabilities` (`CapabilitiesStatusProvider`, reading `Service\EnvironmentProbe`) says what the PHP process is actually allowed to do — its SAPI, whether `exec()` can be called at all, and the ini directives that silently cap an upload or a long task — because a host can withdraw a function from one site and not the next, nothing in the application changes, and the feature relying on it stops without ever raising an error. It is keyed `capabilities` and not `environment`, the report already carrying an `environment` of its own meaning `prod`/`dev`. `registration` (`RegistrationStatusProvider`) says whether a stranger can still create an account here, read off the register `Form`'s own `enabled` flag rather than off a config that could contradict it — a registration opened for a campaign and never closed again looks exactly like one that was never opened, from the outside and from the site's own dashboard. `messenger` (`MessengerStatusProvider`) says how much is waiting in each of the site's queues, and which transport holds what has failed for good - the one state of a site nothing else reports, a stopped worker raising no error, turning no health check red and answering every page normally while mails and scheduled commands stop coming out. Both counts travel because they fail in opposite ways: a message in the failure transport has exhausted its retries and waits to be replayed by hand (`messenger:failed:show`), one piling up in an ordinary transport has never been tried at all - so a site whose worker died shows *zero* failures, and the failure count alone would read as healthy. Transports that cannot count themselves (the scheduler's) are left out rather than reported as zero, "nothing waiting" and "no way to tell" being exactly what this section separates, and `failureTransport` is `null` on a site that configured none - in which case what fails there is dropped rather than kept. A site running without Messenger at all reports the section empty rather than failing.

All three report readings and counts only: `disable_functions` in full names every function a host left reachable, which describes an attack surface rather than a capability, and that stays on the server - and a queue's counts say how much is waiting, never what.
...
## Deprecations — reading the log Monolog isolates

`php bin/console c975l:deprecations:check` reads `var/log/{env}.deprecations.log`, groups the identical messages with their count, and sorts what a Symfony major bump will actually cost you:

```bash
php bin/console c975l:deprecations:check        # the current env
php bin/console c975l:deprecations:check prod   # the log of another env
```

It needs `config/packages/monolog.yaml` to isolate the `deprecation` channel in a handler of its own: without it that log file never exists, and the command warns about the missing file rather than reporting a site clean.

Each message is cross-referenced against the app's own `src/` and every installed c975L bundle's source, and only the messages hitting one of them are printed — a deprecation nobody but a vendor triggers is nothing you can act on. A hit is **ACTIONABLE** when the file names the deprecated class the message quotes, and **To be checked** when it only names the class' package or parent namespace, which a `use Foo\Bar\Annotation as X;` import is the reason to keep: the file references `X\Uploadable` and never spells the full name out. A package name is read as the two segments it is, so a message quoting a template's location (`templates/components/site-header/menu.html.twig`) no longer hands out `templates/components` or `-header/menu` as packages and drags in every file naming that directory.
