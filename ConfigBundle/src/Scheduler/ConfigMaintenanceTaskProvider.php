...
class ConfigMaintenanceTaskProvider implements MaintenanceTaskProviderInterface
{
    public function getMaintenanceTasks(): array
    {
        return [
            ...
            // Expired session rows, nightly: PHP's own garbage collection is a dice roll the handler only prunes on, and on a managed host it can simply never come up
            ...
            // Inactive accounts, weekly: the limit is counted in years and the notice in weeks, a day's precision changes nothing
            new MaintenanceTask('# #(3-6) * * 3', 'c975l:config:users-cleanup'),
