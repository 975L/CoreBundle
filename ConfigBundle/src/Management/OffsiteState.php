...
// When something last *successfully* left this server, and how much of it. Written by whoever performs the
// transfer - c975l:config:backup:offsite when the site pushes, or --ack when an outside machine pulls - and read
// back by c975l:config:backup, so one HealthCheckResult row carries both "the backup ran" and "the backup left",
// which are not the same fact and used to be reported as if only the first existed.
class OffsiteState
{
    ...
    // Only a transfer that worked moves the clock forward. A failed run recorded as "at: now" would read as a fresh
    // copy, and three nights of failures would look like three nights of success - the exact reassurance this file exists to withhold
    // Merges over the previous state rather than replacing it: two streams write here - the nightly mirror, which
    // counts files and bytes, and the archives push every 6 hours, which counts neither - and a plain overwrite
    // dropped whatever the other one had recorded
    public function recordSuccess(string $projectDir, array $details = []): void
    {
        ...
        // A stream only clears the failure it raised itself: the archives push succeeding says nothing about the
        // mirror that failed last night, and clearing it there is how a month of broken mirrors read as "ok"
    ...
    // Hours since the last successful transfer, null when none was ever recorded - which the dashboard reads as
    // "nothing has ever left this server", the worse of the two cases and not one to report as merely stale
