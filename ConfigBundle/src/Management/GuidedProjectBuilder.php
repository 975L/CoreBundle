...
class GuidedProjectBuilder
{
    ...
    // Where the project's film is shown, "site-tutorials-url" and its slug: the address answers a project never shot too (bundles.975l.com sends it on to its films' index), so the link is written without knowing which films exist. Null when the entry is emptied, which removes every "Watch the film" link
    private function filmUrl(string $slug): ?string
    {
        $base = rtrim(trim((string) $this->configService->get('site-tutorials-url')), '/');

        return '' === $base ? null : $base . '/' . rawurlencode($slug);
    }
