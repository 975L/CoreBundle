...
class Config
{
    ...
    private const string THEME_COLOR_PATTERN = '/^[#a-zA-Z0-9.,%()\s-]+$/';

    // Nothing but hex digits, of a length a hex color has: the "#" was left out. The pattern above accepts it (they are all valid characters), and CSS then drops the whole declaration silently - the property falls back to its initial value, which reads as a color of its own rather than as a mistake
    private const string THEME_COLOR_HEX_WITHOUT_HASH_PATTERN = '/^[0-9a-fA-F]{3,8}$/';

    // Theme slugs holding a CSS color value (as opposed to a font stack or the theme-mode choice)
    private const string THEME_COLOR_SLUG_PREFIX = 'theme-color-';

    // Validates that a "theme-color-*" config holds a plausible CSS color, whatever the group it is displayed in - the same slug prefix ThemeVariablesCssListener compiles on, a satellite bundle keeping its own colors in its own group
    #[Assert\Callback]
    public function validateThemeColorValue(ExecutionContextInterface $context): void
    {
        if (!str_starts_with($this->slug, self::THEME_COLOR_SLUG_PREFIX) || null === $this->value || '' === $this->value) {
            return;
        }

        if (
            1 !== preg_match(self::THEME_COLOR_PATTERN, $this->value)
            || 1 === preg_match(self::THEME_COLOR_HEX_WITHOUT_HASH_PATTERN, $this->value)
        ) {
