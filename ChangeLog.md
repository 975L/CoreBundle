...
- **This bundle's own javascript is now run rather than read**, through the harness `c975L/UiBundle` ships (`Testing\JsCase`): the health-check table filtered, sorted and acknowledged over the rows `_table.html.twig` renders, the progress banner followed through a run given up on, and the guided helpers pointed at something that has moved - 46 scenarios over all five of this bundle's scripts, where it had none. All it declares is where its assets are (`tests/Assets/JsCase`) (02/09/2026)
- The two guided controllers are the ones that earn it most, almost nothing they do being visible in the file they are written in: `onboarding-tour.js` appends its overlay and its panel to `<body>`, matches each step against EasyAdmin's own sidebar `a[href]` and answers the arrow keys on the document - so what it leaves behind on the body, and whether a closed tour still answers those keys, is only readable off something that ran it. `guided-project.js` is rebuilt from browser storage on every admin page, and a project resumed one step short, a paused one coming back unasked or a finished one still offering to be resumed are differences of stored state rather than of code (02/09/2026)
- Both are now held to the two guarantees a reader would have to take on trust: a step whose url leads off-site - or reads `javascript:` - is rendered in place instead of followed, that url being written in the back-office; and a browser refusing `localStorage` costs the resume and the badge rather than the parcours (02/09/2026)
- **A config entry carries its value, the code no longer repeats it**: `site-backup-db-host`, `site-backup-max-age-hours` and `site-role-admin` are read as they stand, the `localhost`/`30`/`ROLE_ADMIN` written a second time in `BackupCommand`, `ExportTablesCommand`, `BackupAlertProvider` and `BackupDigestBuilder` being two sources of truth for one setting (01/09/2026)
- An empty `site-backup-max-age-hours` drops the staleness check instead of reading as a threshold of zero, which would have raised a danger alert on every backup older than an hour (01/09/2026)
- A guided project now starts from any button naming it, wherever that button sits: `guided-project.js` delegates the click on the document instead of binding the ones present when it mounts, and only relabels those inside the dashboard list (01/09/2026)
- **A media only used by a page in the bin leaves the media library's gallery**, and the `svg-fonts` check skips it: a binned page keeps its blocks and their medias, which crowded the gallery with files nobody could act on - a link on the gallery shows them back, nothing is deleted, and restoring the page brings them straight back. `MediaUsageProviderInterface` usages carry an optional `binned` flag, `MediaUsageRegistry::getBinnedOnlyMediaIds()` reads the verdict across every provider - a single live usage is enough to keep a media, a provider with no verdict to give does not vote (`BlockMediaUsageProvider` knows a media hangs off a block, never what owns that block), and a media used nowhere is never hidden (01/09/2026)
- New `MediaRepository::findAttachedToBlock()`, which the above reads: only the rows hanging off a block are read - the only ones a binned page can hold on its own - each with its block joined rather than fetched lazily one row at a time (01/09/2026)
- **A link is only reported broken when a GET says so**: a url routed client-side - `bsky.app/intent/compose`, and every share intent or single-page app route like it - answers 404 to the `HEAD` it serves in 200, which had a working share button listed as a dead external link on every page carrying it. `ContentQualityAnalyzer` and `ContentQualityClient::checkLink()` now retry a `HEAD` that answered >= 400 alongside the ones it couldn't conclude on, and keep the GET's verdict - a host that filters this client is still left alone (01/09/2026)
- The offenders behind a content-quality advice line are listed straight away instead of behind a *See the detail* to click, `label.health_check_advice_details` retiring with the disclosure (01/09/2026)
- **`config/reference.php` no longer reads as code changed since deployment**: FrameworkBundle redumps it on every debug-mode container compile - which a deployment running its migrations in a dedicated env does - and the file being versioned, the `intrusion` check warned about it at every run. It is now dropped from what that row reports, being a file nothing ever loads, written for the IDE alone (01/09/2026)
- **New `ConfigDemoFixtureProvider`**: the three missing addresses a demo site is browsed with - one a release moved, one someone mistyped, one the site itself links to - and the redirection the guided project answers the first with. What arrives from outside is the only part of this bundle's screens a dataset writes: the url descriptions and the health check are generated by the commands a deployment already runs (01/09/2026)
- **A menu entry and a guided project step take an optional `narration`**: what the step sounds like spoken rather than read, resolved in the item's own domain suffixed `_narration` and written in French and English alone; without one, the label and the description stand in (01/09/2026)
- `GuidedProjectBuilder` and `OnboardingStepBuilder` resolve it, and the two bundles ship the catalogues their own entries and steps read (01/09/2026)
- **`c975l:scaffold:diff --acknowledge` no longer marks a file it did not write**: the dry run walking the whole scaffold recorded the new hash for every file merely behind, which the next install then read as the site's own work and never refreshed again (01/09/2026)
- A `--path` no scaffold file answers to fails on `--acknowledge` as it already did on the diff, instead of reporting nothing to acknowledge (01/09/2026)
- A scaffold source resolved outside the project - a Composer `path` repository, symlinked - is hashed where it actually sits, rather than prefixed with the project directory a second time (01/09/2026)
- **New `GuidedProjectBuilder::getAllProjects()`**, every parcours whatever role it asks for, where `getProjects()` answers one reader and drops what they could not open: what a documentation of the whole back office reads - the ecosystem's showcase serves Donovan the parcours of every bundle installed on it, and starting one still checks the reader's own role (`getProject()`) (01/09/2026)
- **The edit links a health check run records point at the site rather than at localhost**: EasyAdmin builds an absolute url whenever there is no `AdminContext`, which a console run never has, so the host came from `framework.router.default_uri` - a per-site `.env` nobody sets. `HealthCheckRunCommand` now fills the `RequestContext` from `site-url`, which is already what the checks are run against (01/09/2026)

- Past entries no longer name the maintainer's own site either: the block showcase ones read as the ecosystem's showcase, the Donovan Q&A one as the ecosystem's backend (01/09/2026)
- The fixtures and comments of both bundles no longer carry the maintainer's own sites, books or a client's business: `example.com` and neutral names stand in their place (01/09/2026)
- The offsite backup target reads `storagebox:example.com` everywhere it is shown as an example - the Readme, the config entry's own description in all three languages, `OffsiteSynchronizer`'s comment and the fixtures: a public bundle names nobody's site but the reader's (01/09/2026)

### UiBundle

- **`email_template_body()` reads the recipient's language**: it took whichever row the name matched first, which on a multilingual site was whichever locale happened to be created first. It now resolves like `renderNamed()` does - the recipient's language, the site's own, then the wording the bundle declares - through the new `EmailTemplateRenderer::renderNamedBody()` (02/09/2026)
- **The two "other ..." legal texts leave the configuration**: `site-other-copyright` and `site-other-cookies` were config entries the cookies and copyright models read through `config()` and printed as a section of their own - the very thing an added section does, and does better: it belongs to the model rather than to the site, it is written on the screen where the rest of that document is written, and it lives in the block's own data, which is translated where a config entry is not (their own description said "non traduit"). Two mechanisms for one need; the older one goes (02/09/2026)
- **New `c975l:ui:legal-models:adopt-config-sections`**, which moves what those two entries hold into the model that carried them, then deletes them. Idempotent - the section is recognised by its own identifier - and it never deletes a text with nowhere to go: a site that filled the entry without ever creating the page showing it keeps the row and is told to move it by hand. The line breaks are kept, the models having printed the text through `nl2br`. A one-off, unlike `c975l:ui:email-templates:ensure` beside it in the deployment: it is meant to be taken out, with the command, once the fleet has run it (02/09/2026)

- **A form says what it says in each of the site's languages**: what an admin types in the form builder is text a visitor reads, not a translation key, and it had none - the contact page of a bilingual site was one translated page carrying a form that only spoke French. New `FormTranslator` puts `FormField`'s label, placeholder and default value, and `FormOutput`'s label and unit, in the same table a page's and a block's own texts live in (`Translation::OWNER_FORM_FIELD` / `OWNER_FORM_OUTPUT`) - a form is one form in every language, `Form::$name` being unique site-wide, so a second row was never an option. No migration: the table takes a new owner type as it stands (02/09/2026)
- The public form reads them (`FormSubmissionType`, one query for the whole form rather than one per field), a calculator's results too (`ExpressionEvaluator` for the unit, the `ui_form_label()` Twig function for the label - named apart from Symfony's own `form_label()`) (02/09/2026)
- The back office writes them: the form screen gains the language selector the page screen already had, `?contenu=en` offering the fields and the results as their translatable texts alone, unmapped, the source between brackets marking what is left to do (`FormFieldType`/`FormOutputType`'s `translation_locale`, shared through `FormTranslationBuilder`) (02/09/2026)
- A language screen neither adds nor deletes a row: a form is composed once, and a field taken away from one language was taken away from every language at once (02/09/2026)
- The `ui-form` parcours ends on writing the form in the site's other languages, the selector being the screen's only untaught control (02/09/2026)
- A seeded form carries the wording the declaration already shipped for the site's other languages, so a bilingual site's contact form is readable in both from the moment it is seeded - only ever for a form the run has just created, one already in place having been through the back office since. `SeededTranslationWriteListener` writes them once the caller's flush has given the fields their ids (02/09/2026)
- `TranslationPurgeListener` takes a removed field's or result's translations with it, orphanRemoval firing a removal like any other - left behind, they would be inherited by whatever landed on the same id (02/09/2026)
- Choice options and the links under the submit button are not covered yet: both live inside a JSON column rather than in a column of their own (02/09/2026)

- **New `DemoFixtureLinkerInterface`**, a second pass over a demo dataset once the first has been flushed: the rows that copy an identifier rather than point at one - an order naming the very items it holds - could not be written before, nothing having an identifier yet. Registered like the providers are (`DemoFixtureLinkerRegistry`, `DemoFixtureLinkerPass`), and what it yields is recorded and taken back exactly like the rest (01/09/2026)
- **A form field with no wording no longer answers 500**: a row added to a form's fields and left empty reached the database, where the column refuses a label of nothing - the back office submits without the browser's own check, and nothing on the server refused it. `FormField::$label` carries `Assert\NotBlank` and `Form::$fields` carries `Assert\Valid`, without which the first would never have been read (02/09/2026)
- **The calculator parcours says to name the form**, which it never did: it went straight from the "create" button to the field catalogue, and a form has no name until someone types one - the reader met a field outlined in red and no step explaining it (01/09/2026)
- **New `UiDemoFixtureProvider`**: one review per status, the pending one first, so the moderation screen shows what it is for rather than "no result found". The reviews brought back from a platform are seeded by the bundle that syncs them, the e-mail templates and the health check by the commands that write them (01/09/2026)
- **A `form` block takes the page's step above itself**: the kind's wrapper carried no class at all, which left it the one page-level block outside the vertical rhythm - a form sat flush against the block above it, plainly under a `banner_title`, that one declaring no gap below itself for the next block to lean on. The form, the calculator and the two notices standing in for a closed or already-answered form now open on `.ui-form-block`, which `sass/_forms.scss` gives `--section-space-tight` on its top edge, in the three run-of-blocks contexts alone - a form rendered standalone or by a site's own `render(controller(...))` is spaced by whatever surrounds it, as before (01/09/2026)
- `SectionRhythmTest` locks that step, its three contexts, and the class on each of the four templates the kind renders through (01/09/2026)
- **A `collection` block says which heading its items' titles are drawn with**: the new **Items title level** field, left on *Automatic*, deduces it from the block's own head - `h3` under a titled block, `h2` under one standing on its items alone, which is what a page's `h1` is directly followed by. A listing with no title printed `h3` right under the page's `h1`, skipping a level (01/09/2026)
- The level reaches both presentations - the `card` items through the component's own `level` prop, the `portfolio` ones in place of their hardcoded `h3` - and enters the items' cache key, so two blocks pulling the same source at two levels never serve each other's html (01/09/2026)
- The `portfolio_grid` block gets the same field and the same automatic reading, its projects' `h3` having been hardcoded too; its head is now only printed when a heading or a "see all" link fills it, instead of standing empty over the projects (01/09/2026)
- **Donovan points to the guided project that covers the question asked**: the `donovan-qa` skeleton's context builder writes one prompt section per parcours beside the block kinds, and the model cites one as `tour:<slug>` (01/09/2026)
- A cited parcours is resolved by the site that asked rather than trusted as sent: `AiAssistantClient` turns its slug into a button starting it on the spot, and drops one whose bundle this site doesn't install or whose role this user doesn't hold (01/09/2026)
- The generated context builder reads the guided-project providers directly, `GuidedProjectBuilder` filtering by a user the token-authenticated API call never has (01/09/2026)
- The skeleton's context version moves to `v2`, retiring the answers cached before parcours were part of the context (01/09/2026)
- The maker now also writes a test for the generated context builder, the piece telling a cited parcours apart from a cited block (01/09/2026)
- The Readme and the maker's printed setup no longer name a site as the operator of the shared Donovan backend: the shape is what a self-hosting app needs, and the address of a back office is nobody else's business (01/09/2026)
- **`ui-ai-assistant-rephrase-base-uri` and `-model` are now required whatever the provider**, and no endpoint or model is hardcoded any more: `claude-haiku-4-5`, `gpt-4o-mini` and both providers' URLs are gone from `AiRephraseClient`. An entry left empty keeps the feature off rather than calling a model nobody chose - see UPGRADE.md (01/09/2026)
- The same in the `donovan-qa` skeleton the maker writes, and its printed setup now says the four entries are required (01/09/2026)
- `site-form-delay` is read as it stands in `FormBotProtection`, the 7 seconds repeated in the code being the value its entry already ships with (01/09/2026)
- `ui-block-showcase-url` carries the showcase address alone: an empty entry drops the menu link instead of opening a tab on the ecosystem's own site (01/09/2026)
- A `collection` item's title level is matched against the offered headings before it is drawn, the same rule its two neighbouring templates already state (01/09/2026)
- The generated `donovan-qa` test suite passes as delivered: its two `ask()` stubs were missing the model and base uri its client now requires (01/09/2026)
- Its master-switch test states the whole rule too, the same two entries missing there having made it green whatever `donovan-qa-llm-enabled` held (01/09/2026)
- The `ui-media` parcours says where a media attached to a binned page went, the gallery's own link showing them back (01/09/2026)
- An unset semantic-cache threshold turns the similarity search off in the skeleton, where a defaulted `0.0` matched every question against every other (01/09/2026)
- **A backend that does not answer is said in words, not in its own error key**: `ask()` answering 503 wrote "unavailable" in the log as if the assistant had said it, which is what a reader saw where an answer belonged. `ai-assistant.js` now reveals a message the template carries - translated, and never built from the response - and the caller passes the link it ends on: the assistant's own page from the dashboard, the endpoint's config entry from that page (01/09/2026)
- `AiAssistantErrorMessageTest` covers the hidden message, the link written only where a caller passes one, and the error key never reaching the log again (01/09/2026)
- **New `map` block kind**: a list of places drawn as a map, each placed either by its address or by its GPS coordinates - the second being the only way to pin what no postal address names. The list itself is rendered server-side, every entry carrying a link to the place on OpenStreetMap, and the map is drawn over it: a visitor with no JavaScript, an app that never required `leaflet`, a Google key that was refused, all keep the addresses and their links, which is also the only version a keyboard and a screen reader can work through (02/09/2026)
- Which service draws it is a site-wide setting rather than a field of the block: `ui-map-provider` picks between OpenStreetMap and Google, `ui-map-google-api-key` holds the key the second needs, flagged sensitive. `Map\MapProvider` is where a provider is declared - its tile server, its attribution, whether it needs a key, whether it needs consent - and `ui_map_settings()` hands all of it to the component (02/09/2026)
- Google is held behind the same consent contract as `video_iframe`, writing cookies of its own and being billed per load, where OpenStreetMap's tiles are never gated: they write none. The `sensitive` flag hides the key in the back office and nothing more - a browser key is necessarily published in the page, and what protects it is the HTTP-referrer restriction set on it (02/09/2026)
- An address is geocoded once, when the block is saved (`MapGeocoder`, Nominatim, aliasable through `MapGeocoderInterface`), and the coordinates are stored beside it - so a page carrying a map never geocodes anything while a visitor waits, and a block saved again is not geocoded again. Nominatim whatever the provider the site draws with: a Google key restricted by referrer is refused on a server-to-server call (02/09/2026)
- **A Google map no longer costs a site its `style-src`.** Google's API injects `<style>` elements a nonced policy refuses, and this bundle's own README said the only way out was `'unsafe-inline'` - which is wrong: [Google's Content Security Policy guide](https://developers.google.com/maps/documentation/javascript/content-security-policy) says the API copies the nonce of the first `<script>` and the first `<style>` it finds onto everything it injects, and its strict example is `style-src: 'nonce-…'` with no `'unsafe-inline'` at all. `map.js` now reads `<meta name="csp-nonce">`, puts it on the loader it appends and lays an empty `<style>` carrying it in front. A page serving no policy answers `""` and nothing is appended. **No site has to widen anything for a map**, and a client switching `ui-map-provider` in the back-office changes no deployment file (02/09/2026)
- **And the block says it itself, in place**: whoever placed a map is looking at the page, not at a health check they have no reason to open. `ui_map_settings()` now answers `diagnostic`, true for a user granted the back-office, and the component renders a notice for them alone - a Google map with no key says so without a controller being mounted at all, and a map whose library never loaded says why once it has failed. `map.js` listens for `securitypolicyviolation` and names the directive that refused Google's host, telling a refused policy apart from a key Google turned down - the two look identical to a script otherwise. A visitor's page carries neither the sentences nor the element they are written into: a missing key and a security policy are not theirs to fix, and the list of places is the content either way (02/09/2026)
- What that notice deliberately does not cover: a map that draws over a policy naming its script host and not its tiles is a grey grid nothing in the page can see. That one stays the health check's to raise (02/09/2026)
- **New `MapCspHealthCheckProvider`**, silent on a site drawing over OpenStreetMap and only then: the provider is picked in a back-office, and a policy widened for a Google map nobody asked for is a policy widened for nothing. On a site that did turn it to `google` it reads the site's own `Content-Security-Policy` and names the origins it does not - per directive, `script-src` left to a `'strict-dynamic'` nonce being asked for none. Without it the browser refuses the fetch in silence and all that stays on screen is the list of places (02/09/2026)
- The `ui-map-provider` description says so too, in the three catalogues: Google needs a policy naming its hosts, beside the key, the billing account and the consent. `'unsafe-eval'` and `blob:` appear in Google's own recommended sets and are deliberately *not* written there - whether the narrow use this block makes of the API trips them is unverified, and their list covers Places and Street View too (02/09/2026)
- **Leaflet is served by the bundle**, not required by each app: `public/js/leaflet.js` and its stylesheet are appended on demand by the controller, the way `cookie-consent.js` already serves its own banner - a consuming site gets a working map on the bundle's own update, no third party receives the visitor's ip, and a page carrying no map downloads none of it. The stylesheet is waited for before the map is built, Leaflet reading the sizes its own rules give the panes as it builds (02/09/2026)
- The origins the providers need are exposed as three container parameters rather than one - `c975l_ui.map.img_origins`, `.script_origins`, `.connect_origins` - the way `c975l_ui.video.embed_origins` already was. Three and not one because a single list holding a tile server and a script host at once would authorise scripts from that tile server the day a site named it in its `script-src`; a site staying on OpenStreetMap names the image one alone, Leaflet being served from `self` (02/09/2026)
- **New `config/vendor-assets.json`**, the one place saying which version of Leaflet, `vanilla-cookieconsent` and `canvas-confetti` this bundle serves, with the url each file is fetched back from - three libraries that went through no package manager and that nothing said anything about until now. `bin/vendor-assets.sh` reads it to say what npm has published since, and to fetch a named version back; `VendorAssetsTest` checks offline, at every commit, that each library really carries the version the manifest claims, and that it still carries the api this bundle calls on it - the version check alone passes on any file bearing the new number, working or not (02/09/2026)
- **`LeafletContractTest` runs the vendored library for real**, in headless Chrome, through the very sequence `map.js` drives it with, and reads back the DOM it built: its panes, its tile elements, the two `ui-map__pin` markers, the framing `fitBounds` produced, a popup's escaped text. No tile is ever fetched - the url points nowhere on purpose - so it runs offline in about a second. The first test of this bundle to drive a browser, hence a `browser` group and a skip when `chrome-php/chrome` or the binary is absent (02/09/2026)
- **This bundle's javascript is now run rather than read**: new `Testing\JsCase` mounts a fragment of markup in that same headless Chrome, starts the named controllers over it and reads back what they made of the DOM - 141 scenarios over 17 of this bundle's own scripts and the three libraries it vendors, where 71 of the 72 tests under `tests/Assets/` until now read those scripts as text, passing on a line sitting in a branch nothing reaches and failing on a rename that changed no behaviour. A real browser and not a DOM emulated in node, because 24 of this bundle's 59 scripts measure a layout or watch one, and an emulated DOM answers 0 to every measure without erroring: `readmore.js`'s `scrollHeight <= clientHeight + 1` then reads `0 <= 1` and the suite goes green over a broken controller. No `package.json` comes with it - `chrome-php/chrome` was already a dev dependency (see `LayoutAuditor`) (02/09/2026)
- **The harness now serves this bundle's assets whatever the bundle under test**, and resolves the bare `@c975l/ui-bundle/...` specifiers a satellite's controller imports the way `importmap.php` does (see `ImportmapProvider`: the name is an alias, not a path). Without it, `PaymentBundle/assets/js/handlers.js` and `GalleryBundle/assets/js/gallery-media-sort.js` - the two that borrow from here by name - could not be loaded at all outside this repository (02/09/2026)
- `window.open` joins `matchMedia`, `fetch` and `XMLHttpRequest` among the globals snapshotted when the page opens and put back between scenarios: a share button's popup is stood in for rather than opened, a window really opening being one that would sit over every scenario that follows (02/09/2026)
- **Six bundles now extend `Testing\JsCase`** - ConfigBundle, SiteBundle, BookBundle, PaymentBundle, SocialBundle and GalleryBundle - each declaring where its assets are and nothing else, and each running its own scenarios in its own suite (02/09/2026)
- One browser and one page for the whole run, served over `http://127.0.0.1` from a temporary docroot: a `file://` document has no cookie jar, so vanilla-cookieconsent remembered nothing, and a module served from a `data:` url cannot resolve the relative import its neighbour is behind. A scenario on that shared page costs about a millisecond, the whole group about 35 seconds, and nothing leaves the machine - tile servers and Google's endpoints are pointed at an address that refuses the connection (02/09/2026)
- Stimulus is vendored beside it in `src/Testing/` for it, declared in `config/vendor-assets.json` with a new `"scope": "tests"` saying it reaches no site, and pinned by a `sha256` rather than by a version banner: `@hotwired/stimulus` 3.2.2 still ships one reading "Stimulus 3.2.1", so no marker could prove which release is on disk. `VendorAssetsTest` accepts either, and `bin/vendor-assets.sh` recomputes the digest when it bumps one (02/09/2026)
- **The harness ships, so every bundle depending on this one runs its javascript the same way**: `Testing\JsCase` lives in `src/` rather than in `tests/`, for the reason `StylesheetCascade` beside it already gives - a bundle's `tests/` is autoload-dev and never reaches the bundles depending on it. A satellite declares where its assets are and nothing more; its controllers are then served beside this bundle's, the bare `@hotwired/stimulus` they import rewritten towards the vendored copy. Each bundle's tree is copied under the docroot the moment one of its tests first asks for it, and served under a name of its own: the server lives for the whole run, and a shared name would belong to whichever bundle's class happened to start it (02/09/2026)
- **The tests that read a script as text were narrowed rather than deleted**: fifteen of them asserted lines of implementation the scenarios now prove - `setAttribute("aria-current", "true")`, `this.observer?.disconnect();`, a `for` loop and the `break` inside it. What stayed is what a scenario cannot see, because it mounts markup of its own: the barrel entry, the `data-controller` and the `data-*-target` a template writes, the class a stylesheet paints, the value a PHP service and a controller must spell alike - and every guarantee of *absence*, which no scenario can demonstrate. Where a guarantee had no scenario, the scenario was written instead: a text re-measured when its box narrows, a consent banner registered under the other spelling, a player four thousand pixels below the fold, an autocomplete the form declared none of, and the tally a one-icon or compact rating widget writes (02/09/2026)
- That half earned its keep at once: `templates/components/Map/Map.html.twig` writes `ui-map`, and the first behaviour test drove the controller as `map` - which Stimulus accepts, and which no scenario would ever have caught (02/09/2026)
- **`video-iframe.js` now hears a consent withdrawn by a visitor who had already accepted**: `connect()` returned before ever attaching the banner's listeners on that path, so a withdrawal left the play button on screen offering a video just refused - the click was still checked against the answer of the moment, so nothing was ever framed without consent, but the button had no business being there. Found by `VideoIframeBehaviourTest` (02/09/2026)
- **Duplicating a media or a nested row no longer gives the copy its source's position**: `copyMediaValues()` and `copyItemValues()` copied the `[position]` field along with the values, undoing the renumbering run a line earlier and leaving two rows claiming the same place - an order nothing can resolve on save. Found by `BlockDuplicateBehaviourTest` (02/09/2026)
- The three images `leaflet.css` names travel with it (`public/css/images/`): AssetMapper resolves a stylesheet's `url()` strictly, so one it cannot find fails the compilation of every bundle stylesheet at once - a single missing PNG answered 500 on the Ui, Payment and Gallery sheets together. `MapControllerTest` now reads the sheet's own `url()` calls back and checks each is shipped and declared (02/09/2026)
- **A `contact_details` block builds its map link instead of being handed one**: the new *Build the Google Maps link* box has `GoogleMapsLinkBuilder` assemble it from the coordinates, or failing that from the postal address, so nobody has to go to Google, search for their own business and paste the url back. A plain Maps url - no key, no billing account, no script loaded - and the map field still wins when both are filled. Resolved once in the component, so the button and the graph's `hasMap` can never point at two different places (02/09/2026)
