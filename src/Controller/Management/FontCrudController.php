...
use c975L\UiBundle\Management\FontExportProvider;
use c975L\UiBundle\Management\FontImportProvider;
...
class FontCrudController extends AbstractCrudController
{
    ...
    {
        return $crud
            ->setEntityLabelInSingular(t('label.font', [], 'ui'))
            ->setEntityLabelInPlural(t('label.fonts', [], 'ui'))
            ->setEntityPermission($this->configService->get('site-role-editor'))
            ->setDefaultSort(['name' => 'ASC', 'weight' => 'ASC'])
            ->showEntityActionsInlined()
            ->overrideTemplate('crud/index', '@c975LUi/management/font_crud_index.html.twig')
            ->overrideTemplate('crud/edit', '@c975LUi/management/font_crud_edit.html.twig')
            ->overrideTemplate('crud/new', '@c975LUi/management/font_crud_new.html.twig')
        ;
    }
    ...
    public function configureActions(Actions $actions): Actions
    {
        ...
        $actions->add(Crud::PAGE_INDEX, Action::new('exportSelection', t('action.export_selection', [], 'ui'), 'fa fa-file-export')
            ->createAsBatchAction()
        ...
        $actions->add(Crud::PAGE_INDEX, Action::new('bulkImport', t('action.font_bulk_import', [], 'ui'), 'fa fa-upload')
    ...
    public function configureFields(string $pageName): iterable
    {
        ...
        foreach (self::WEIGHT_NAMES as $value => $labelKey) {
            $label = Font::WEIGHT_VARIABLE === $value
                ? $this->translator->trans($labelKey, [], 'ui')
                : sprintf('%d - %s', $value, $this->translator->trans($labelKey, [], 'ui'));
        ...
        return [
            ...
            ChoiceField::new('weight')
                ->setLabel(t('label.font_weight', [], 'ui'))
                ->setChoices($weightChoices)
                ->setRequired(true),

            ChoiceField::new('style')
                ->setLabel(t('label.font_style', [], 'ui'))
                ->setTranslatableChoices(array_combine(
                    self::STYLE_CHOICES,
                    array_map(fn (string $labelKey) => t($labelKey, [], 'ui'), array_keys(self::STYLE_CHOICES)),
                ))
                ->setRequired(true),

            Field::new('file')
                ->setLabel(t('label.file', [], 'ui'))
                ->setHelp(t('label.font_file_help', [], 'ui'))
                ...
                ->setFormTypeOptions([
                    ...
                    'constraints' => [
                        new File(
                            ...
                            extensionsMessage: $this->translator->trans('label.font_file_invalid_extension', [], 'ui'),
                        ),
                    ],
                ])
                ->onlyOnForms(),
