...
use c975L\UiBundle\Form\LegalModelCustomizationType;
...
class LegalModelController extends AbstractController
{
    ...
    public const INDEX_ROUTE = 'management_ui_legal_models';
    public const CUSTOMIZE_ROUTE = 'management_ui_legal_model_customize';
    ...
    public function index(Request $request): Response
    {
        ...
        foreach ($blocks as $block) {
            ...
            $rows[] = [
                ...
                'block' => $block,
                'model' => $model,
                // customize() 404s on a model this bundle no longer ships (an imported block, a model dropped by an update), so the row must not offer a link into it - the block is still listed, that being how one finds out it needs its model reassigned
                'known' => $this->catalog->has($model),
        ...
        return $this->render('@c975LUi/management/legal_model_index.html.twig', [
            'rows' => $rows,
    ...
    #[AdminRoute(
        path: '/ui/legal-models/{block}',
        name: 'ui_legal_model_customize',
    ...
    public function customize(Block $block, Request $request): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->addFlash('success', $this->translator->trans('flash.legal_model_customized', [], 'ui'));

            return $this->redirectToCustomize($block);
        }

        return $this->render('@c975LUi/management/legal_model_customize.html.twig', [
