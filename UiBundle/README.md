...
The sidebar's "Links" section (see `c975L\UiBundle\Management\MenuProvider::getLinks()`) instead links out to <https://bundles.975l.com/pages/blocks>, the c975L ecosystem's own canonical showcase of every bundle's block kinds - rendered inline in a normal page, no iframe, no CSP conflict. That url is the `ui-block-showcase-url` config entry, pre-filled with the ecosystem showcase: an app hosting its own showcase page just changes the value, and an entry left empty drops the menu item altogether rather than opening a tab on an address the code chose - nothing here repeats the url in PHP.
