...
It also implements ConfigBundle's `Management\GuidedProjectProviderInterface` (`Management\UiGuidedProjectProvider`, auto-tagged the same way), contributing seven replayable exercises to the dashboard's "Guided projects" panel: **"Corriger ce qu'une image dit d'elle-même"** (give a picture its alternative text and its credits in the library every image of the site ends up in), **"Mettre en place les graphiques du site"** (the handful of images a site is recognised by — favicon, logo, error image — which nothing else puts in place), **"Ajuster un document légal"** (hide, retitle or rewrite a section of a legal model this bundle ships, and add sections of your own, without losing the updates it keeps receiving), **"Faire reformuler un texte par Donovan"** (put the rephrasing key in place, then try it on the screen's own textarea before pressing that button under a real field), **"Créer un formulaire"** (build it field by field, then drop it into a page through its block), **"Personnaliser un e-mail"** (rework a sent e-mail's content, made of the same blocks a page is) and **"Installer une famille de polices"** (drop the whole family in at once and let the site compile its stylesheet from it).
