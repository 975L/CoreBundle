...
- To answer "how do I do this" rather than "which block", add the guided projects to that same prompt - every `GuidedProjectProviderInterface` your backend installs, with each project's slug, label, description and step labels - and have the model cite one as `tour:<slug>`. Read the providers directly (`#[AutowireIterator('c975l.guided_project_provider')]`) rather than through `GuidedProjectBuilder`: that one filters by the current user's role, and a token-authenticated API call has no user at all. Return such a citation as `{label, url: '', project: '<slug>'}` and the asking site turns it into a button starting that parcours - a slug it doesn't know is dropped, which is what makes one backend answerable for sites installing different bundles.
