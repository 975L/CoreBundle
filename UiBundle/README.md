...
The band's own ink is `var(--card-accent-color, #fff)`, that fallback being a stated `#fff` and never `var(--white)`, for the reason given under [colored backgrounds](#colored-backgrounds): the band is a hue in both color modes, and dark mode swaps that token with `--black`. Four of the twelve — orange, yellow, lime and teal — are light enough that white text falls under 4.5:1 on them, so they also set `--card-accent-color: #000` and `--card-accent-invert: 0`, which darkens the header's text and stops its icons being inverted to white. A site retuning one of the other eight towards a light hue has to carry those two along:
