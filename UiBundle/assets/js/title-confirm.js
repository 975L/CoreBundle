...
// Confirms with the user before letting them edit the field, since it will also change the record's slug, hence its public url (mounted by SiteBundle's PageCrudController and GalleryBundle's GalleryCategoryCrudController). Lives here rather than in either of them: the behavior is theirs to trigger, not to own, and every back office loads this app anyway. Reuses EasyAdmin's own action-confirmation modal (unconditionally rendered on every crud/edit page, see vendor/easycorp/easyadmin-bundle .../_action_confirmation_modal.html.twig) instead of a native confirm() - same look as the "move to trash" confirmation, and window.bootstrap is already exposed globally by EasyAdmin's own admin.js.
