...
**The flash shown after a submission is picked on the action's key**, in `FormController::submit()`:
`send_email` says the message was sent, `register` says a confirmation email may not have been resent
(`EmailVerifier` holds an address for an hour), everything else keeps the generic `label.form_submitted`.
An action whose success genuinely needs its own wording adds a branch there and a key in the `ui` domain —
`Form` carries no per-row success text.

**Where a successful submission leads** is back to the page the visitor came from, with that flash,
unless a page of its own is named — then the visitor is redirected there and the flash left out. An
action implementing **`Contract\SuccessUrlFormActionInterface`** answers `getSuccessUrl()` after a
`handle()` returning `true` (the page of what it just created; it implements `ResetInterface`, since it
keeps that url between the two calls); otherwise the **`successUrl`** key of `actionConfig`, which an
editor sets for any action (a thank-you page). A failed action never leads to either.

**The submit button's wording** is the generic `label.send` unless the **`submitLabel`** key of
`actionConfig` names one — a single text, or one per language (`{"fr": "Je raccourcis", "en": "Shorten"}`),
a language the map misses falling back on `label.send` (`Form::getSubmitLabel($locale)`).
...
- **Two `choice` options never share a value**: `FormField::validateOptionValues()` refuses it on
  `optionsText`, since Symfony's `ChoiceType` then submits 0, 1, 2... and the formula reads those instead.
...
- The results are rendered **server-side** before any script runs; `assets/js/calculator.js` only
  keeps them in step, debounced, sending the number, range, checkbox and select controls alone
...
- A `checkbox` is a numeric type (`FormField::NUMERIC_TYPES`): a yes/no switch reading `1` ticked and
  `0` otherwise, sent by its state even unticked, its default applying only when nothing was sent.
- A field given a `price` (`FormField::$price`) reads as its value times that price in a formula — a
  ticked box reads its price — and shows it after its label, on the page and in the email, written by
  `Service\PriceFormatter`, the one writer of an amount. Never repeat that price inside a formula.
  A priced `choice` shows value × price after each option instead (`PriceFormatter::optionLabel()`),
  never after its own label, and leaves an option worth 0 or a non-numeric one bare.
- An output ticked `hiddenWhenZero` (`FormOutput::$hiddenWhenZero`) is a line of a detailed quote: hidden
  on the page (`data-ui-calculator-hide-zero`, toggled by `calculator.js`) and left out of the email while it is 0.
...
- `SendEmailFormAction` writes a `choice` by its option's **label** and a `checkbox` as a translated
  `label.yes`/`label.no` (for every form), and appends each
...
(`site-form-delay`, which a form overrides with `minDelay` in its `actionConfig`), a site-wide GDPR information line (`url-privacy-policy`), a live MX/A DNS check on every
...
- **Do not multiply a priced field by its price again in a formula** — it already reads as value × price.
