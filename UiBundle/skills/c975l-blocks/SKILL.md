...
- **A collection is rendered there with its entries and without Add or Delete**: a page is composed
  ...
  away from every language at once. The block's own purge only ever drops what the source no longer
  says at that place, compared on the words alone so a rich text editor's re-serialisation is not read
  as a rewrite.
