...
year. The directory is the `c975l_ui.build_dir` parameter (relative to `public/`, edge slashes
trimmed at compile time): a demo context sets its own so its flushes don't overwrite the site's.
...
**An email rule states its color, it does not read a token mixed out of the palette.** `sass/emails/`
is inlined by `resolve_css_variables`, which substitutes the *admin's* palette and computes any
`color-mix()` right before sending — while the message keeps the plain white `emails/_base.scss` fixes.
So a token derived from `--text`/`--background` (`--label-color`, `--input-placeholder-color`) lands a
near-white grey on white for any site with a dark palette. The blanket `*` rule and `.text-muted` both
state a hex for that reason, and `EmailStylesheetTest` fails if either goes back to a themed color.
...
**The family, the weight and the style are read off the uploaded file's own name** by
`Service\FontFilenameParser`, so the face a theme references is whatever the filename declares — not a
field an editor fills. It reads several trailing segments and not the last one alone (Google puts the
style before the axis segment, `SourceSans3-Italic-VariableFont_wght.ttf`), and it splits the family on
an acronym and on a number: `IBMPlexMono` gives `IBM Plex Mono`, `SourceSans3` gives `Source Sans 3`.
A face whose family name comes out wrong answers to nothing and the browser falls back on a generic —
so a font that renders as the wrong face is a filename question first.
...
`paginate()` takes the listing **whole**, as an array, and cuts the page out of it - for a caller
reading its rows and sorting them in php. A listing too long for that reads its page and its total in
SQL and hands both to `Paginator::paginateSlice()` (`$pageItems, $page, $perPage, $total`), `paginate()`
being only `array_slice()` in front of it. Both return a `Model\Pagination`, countable and
...
| `Service\Paginator::paginate()` | cuts a listing into pages and says what the next page's url is built from - the paginator of the ecosystem, KnpPaginatorBundle being gone; `paginateSlice()` for a page and a total already read in SQL |
