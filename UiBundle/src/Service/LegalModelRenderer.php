...
class LegalModelRenderer
{
    ...
    private function templatePath(string $model, string $locale): string
    {
        ...
        return $this->twig->getLoader()->exists($path)
            ? $path
            : sprintf('@c975LUi/models/%s.%s.html.twig', $model, LegalModelCatalog::FALLBACK_LOCALE);
    }
    ...
    private function reorder(\Dom\Element $container, array $units, array $positions): void
    {
        ...
        foreach ($units as $rank => $unit) {
            $ranked[] = [
                // Its natural index in this scope, which is the very scale the stored positions are on (dense, 0-based, per scope - see LegalModelCustomizer::reorderedScopes()). Ranking an un-positioned unit above every stored one instead, as a *10 scale did, sent a section a bundle update had just added to the end of the document, while the customization screen went on listing it where the bundle put it
                'position' => isset($positions[$unit['id']]) ? (float) $positions[$unit['id']] : (float) $rank,
                'rank' => $rank,
                'nodes' => $unit['nodes'],
            ];
        }
