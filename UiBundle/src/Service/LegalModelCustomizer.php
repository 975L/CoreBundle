...
class LegalModelCustomizer
{
    ...
    public function ordered(array $units, array $positions): array
    {
        ...
        foreach ($units as $rank => $unit) {
            ...
            // Its natural index within its own scope, not in the flat list: stored positions are dense 0-based per-scope indices (see reorderedScopes()), so a unit nobody ever dragged only slots back in where the bundle put it when it is measured on that same scale. LegalModelRenderer::reorder() sorts on the identical one, which is what keeps the screen and the published page showing the same order
            $scopeRank = $scopeRanks[$scope] = ($scopeRanks[$scope] ?? -1) + 1;

            $ranked[] = [
                'scope' => $scope,
                'position' => $positions[$unit['id']] ?? $scopeRank,
                'rank' => $rank,
                'unit' => $unit,
            ];
        }
