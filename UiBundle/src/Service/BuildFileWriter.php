...
// The one way a listener drops a generated stylesheet into the build directory ("c975l_ui.build_dir" under public/), this bundle's own compiled-stylesheet directory (see StylesheetCacheWarmer) - used by FontCssListener here and by ThemeVariablesCssListener, which both rewrite their whole file once per flush. Static and stateless rather than the trait this used to be: a trait shared across bundles is only ever analysed against the users living in the same package, so its callers' own $projectDir read nowhere else looked write-only
class BuildFileWriter
{
    ...
    public static function write(string $projectDir, string $buildDir, string $filename, string $contents): void
    {
        $buildDir = $projectDir . '/public/' . $buildDir;
