...
// Where this bundle's uploads land, as UiMediaNamer writes them. A path that isn't on disk is skipped, so an install
// using none of these declares nothing extra and reports nothing missing
class UiBackupPathProvider implements BackupPathProviderInterface
{
    // A role with no fixed icon spec keeps whatever it was uploaded as: raster is converted to webp by the namer, and
    // anything else (an SVG logo) keeps its own extension. Both are declared, the one that isn't on disk being skipped
    ...
    public function getBackupPaths(): array
    {
        ...
        // The site-wide singleton graphics, which UiMediaNamer deliberately writes at the root of public/ under the
        // role's own name rather than under medias/ - so no folder declaration reaches them, and a backup that only
        // looked at medias/ silently left a site with no favicon and no logo to be restored from. Archived rather
        // than mirrored: a handful of small files, worth a history, and cheap to carry on every run.
