...
// The accent color a block can be marked with, picked from twelve fixed hues rather than from a color
// input: a free value would put arbitrary CSS in the database, and a per-site vocabulary ("socle",
// "sale"...) would tie a look to a subject that changes. A hue names itself and stays true whatever the
// cards end up being about. Each one is a --block-accent-* token (see sass/_tokens.scss), so a site
// retunes any of them from its own theme.css without the stored value ever changing
