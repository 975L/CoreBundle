...
{
    // Matches the ".flex-columns--{alignment}" modifiers styled in sass/_page-sections.scss; "top" is the row's
    // own default and writes no class, so every row stored before this field existed goes on rendering as it did
    ...
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        ...
        // Columns of unequal height hang from the top of the row by default, which is what a row of cards or of
        // stacked sections wants. A row pairing one long column with one short one - a lead and the paragraph
        // answering it - reads as one statement instead, and the two have to be set against each other's middle
