    ...
    private function fieldOptions(FormField $field, bool $required, bool $prefilled, array $constraints): array
    {
        return [
            ...
            'attr' => array_filter([
                ...
                // A control a formula reads is never restored by the browser on reload (Firefox does, silently): the page then shows its defaults or what its address carries, and the results printed for them - name and email keep their autofill. A checkbox is left out, HTML allowing no "autocomplete" on it: one Firefox restores is caught by calculator.js, which asks the results again
                'autocomplete' => FormField::TYPE_PASSWORD === $field->getType() ? 'new-password' : ($field->isNumeric() && FormField::TYPE_CHECKBOX !== $field->getType() ? 'off' : null),
