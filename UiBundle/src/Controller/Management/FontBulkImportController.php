...
class FontBulkImportController extends AbstractController
{
    ...
    public function index(Request $request): Response
    {
        ...
        return $this->render('@c975LUi/management/font_bulk_import.html.twig');
    }

    private function handleImport(Request $request): Response
    {
        if (!$this->isCsrfTokenValid(self::ROUTE, $request->request->get('_token'))) {
            $this->addFlash('danger', $this->translator->trans('flash.font_bulk_import_invalid_token', [], 'ui'));
        ...
        if ([] === $files) {
            $this->addFlash('danger', $this->translator->trans('flash.font_bulk_import_no_files', [], 'ui'));
        ...
        if ([] !== $skipped) {
            $this->addFlash('warning', $this->translator->trans('flash.font_bulk_import_skipped', ['%files%' => implode(', ', $skipped)], 'ui'));
        }
        ...
        $this->addFlash('success', $this->translator->trans('flash.font_bulk_import_success', ['%count%' => $created], 'ui'));
