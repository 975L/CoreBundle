...
class ShareImageMetaTest extends TestCase
{
    ...
    private function renderImageBlock(array $context): string
    {
        ...
        $this->assertSame(
            1,
            preg_match('/(\{% if ogImage is defined %\}.*?\{% endif %\})\s*\{% set canonicalUrl/s', $layout, $matches),
            'The share image block is no longer written between "{% if ogImage is defined %}" and the canonical url in layout.html.twig, this test can no longer read it.'
        );
