...
class UrlMetadataFallbackTest extends TestCase
{
    ...
    // A layout extending this one resolves its own entity's image and hands the media over, which opens the cascade ahead of the row (see @c975LSite/layout.html.twig and its Page)
    public function testAChildLayoutsShareImageWinsOverTheRow(): void
    {
        $rendered = $this->render($this->row(ogImage: 'listing.jpg'), ['ogImageMedia' => self::media('page.jpg')]);
    ...
    {
        $layout = (string) file_get_contents(dirname(__DIR__, 2) . '/templates/layout.html.twig');

        $this->assertSame(
            1,
            preg_match('/(\{% set urlMetadata = url_metadata\(\) %\}.*?)\{% set siteName/s', $layout, $matches),
