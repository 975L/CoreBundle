...
class LegalModelRendererTest extends TestCase
{
    ...
    // A bundle update adding a section to an already-reordered scope leaves that one un-positioned. It used to be ranked above every stored position and pushed to the end of the document, while the customization screen went on listing it where the bundle put it - the two now read the same scale (see LegalModelCustomizer::ordered())
    public function testAnUnpositionedUnitKeepsItsNaturalPlaceAmongPinnedSiblings(): void
    {
        $html = $this->renderer()->render(self::MODEL, null, ['positions' => ['one' => 0, 'two' => 1]], 'fr');

        $this->assertLessThan(strpos($html, '<h2>One</h2>'), strpos($html, 'Intro of Acme'));
        $this->assertLessThan(strpos($html, '<h2>Two</h2>'), strpos($html, '<h2>One</h2>'));
    }
