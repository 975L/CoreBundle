...
class LegalModelCustomizerTest extends TestCase
{
    ...
    // A bundle update adding a section to an already-reordered scope leaves that one section un-positioned. Measured against the stored positions - dense 0-based per scope - it has to keep its natural place, and the very same place LegalModelRenderer::reorder() gives it, or the screen and the published page disagree
    public function testAnUnpositionedSubSectionKeepsItsNaturalPlaceAmongPinnedSiblings(): void
    {
        $customizer = $this->customizer(self::TEMPLATE);
        $units = $customizer->units(self::MODEL, 'fr');

        // "two.b" was dragged before a sibling that no longer exists, "two.a" never carried a position of its own
        $ordered = $customizer->ordered($units, ['two.b' => 1]);
    ...
    private function customizer(string $template): LegalModelCustomizer
    {
        ...
        $twig = new Environment(new ArrayLoader([
            '@c975LUi/models/' . self::MODEL . '.fr.html.twig' => $template,
        ]));
