...
> This bundle's own upgrade notes stop at `v1.17.0`, its last published release. It now ships inside
> `c975l/core-bundle`, whose [UPGRADE.md](../UPGRADE.md) carries everything that came after.
...
`contact_details` asks for the same information plus an e-mail, a website, a map link and coordinates, renders it as a readable panel, and publishes a schema.org graph as JSON-LD alongside it. Every field is optional; the one that carries weight is the **name**, without which no graph is emitted at all and the block renders as a plain display block. Opening hours are now a collection of ranges (days + opening + closing), one row per range, so a business closing for lunch fills in two.

**If your application stored `rich_snippet` blocks**, they now render as nothing: `Twig\BlockExtension::renderBlock()` skips a block whose kind is no longer registered instead of letting the registry throw `Unknown block` and take the whole page with it - which is what it did until this release, for any dropped kind, not just this one. There is no automatic migration: the two data shapes differ (`itemscope` held a full URL where `schemaType` holds a bare type name, `addressAddressLocality` is now `addressLocality`, and the free-text `openingHours` line has no equivalent), so re-enter them as `contact_details` blocks. **If your templates called the component directly**, replace `<twig:c975LUi:General:RichSnippet>` with `<twig:c975LUi:Contact:Details>` - its props are listed at the top of `templates/components/Contact/Details.html.twig`. **If your stylesheet overrode `.rich-snippet`**, retarget it at `.contact-details`.

Two things follow, neither needing action: the `.banner-title` block now takes a `1em` bottom margin, the full-bleed breakout having left the next block flush against it; and a site running a nonce-only CSP has to nonce the block's `<script type="application/ld+json">` in its own template override, the tag carrying none.
